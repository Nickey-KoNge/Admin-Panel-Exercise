...
//get data for dropdownlist
  ...
  const leaveType = useMemo (
    () => leavetype.map((leavetype) => ({value:leavetype.id, label: leavetype.name})),
    [leavetype]
  );
  const modeType = useMemo (
    () => modetype.map((modetype) => ({value: modetype.id, label: modetype.name})),
    [modetype]
  );
//submit data
  ...
  if (!session?.user?.id) {
    ...
    try{
      ...
      const newLeaveRequest = {
        ...
        type: leaveType.find(opt => opt.value === formData.leaveTypeId)?.label || '',
        mode: modeType.find(opt => opt.value === formData.modeId)?.label || '',
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className={styles.form}>
      ...
      <div className={styles.fromGroup}>
        ...
        />
        {errors.leaveTypeId && <p className={styles.errorText}>{errors.leaveTypeId.message}</p>}
      </div>
      <div className={styles.formGroup}>
        ...
        />
        {errors.modeId && <p className={styles.errorText}>{errors.modeId.message}</p>}
      </div>
        ...
        <div className={styles.formGroup}>
            ...
            />
            {errors.requestDate && <p className={styles.errorText}>{errors.requestDate.message}</p>}
        </div>
        <div className={styles.formGroup}>
            ...
            <input 
                type="number"
                step="0.5"
                {...register("noofday", { required: "Number of days is required", valueAsNumber: true })}
            />
            {errors.noofday && <p className={styles.errorText}>{errors.noofday.message}</p>}
        </div>
      </div>
      <div className={styles.formGroup}>
        ...
        />
        {errors.reason && <p className={styles.errorText}>{errors.reason.message}</p>}
      </div>
